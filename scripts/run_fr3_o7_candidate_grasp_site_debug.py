...
def site_id(model, site_name):
    sid = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, site_name)
    if sid < 0:
        raise RuntimeError(f"cannot find site: {site_name}")
    return sid


def frame_pose(model, data, frame_type, frame_name):
    if frame_type == "body":
        bid = v12.body_id(model, frame_name)
        return data.xpos[bid].copy(), data.xquat[bid].copy()

    if frame_type == "site":
        sid = site_id(model, frame_name)
        pos = data.site_xpos[sid].copy()
        R = data.site_xmat[sid].reshape(3, 3).copy()
        quat = v12.rotmat_to_quat(R)
        return pos, quat

    raise RuntimeError(f"unsupported frame_type: {frame_type}")


def frame_jac(model, data, frame_type, frame_name):
...
def solve_franka_ik_dls_frame(
    ...
    for j, val in seed_q.items():
        v12.set_qpos_joint(model, data, j, val)
    ...
    best_err = 1e9
    best_q = {j: seed_q[j] for j in v12.FRANKA_JOINTS}
    best_info = {}

    for it in range(max_iter):
        mujoco.mj_forward(model, data)

        cur_pos, cur_quat = frame_pose(
            model,
            data,
            target_frame_type,
            target_frame_name,
        ...
        if err_norm < best_err:
            best_err = err_norm
            best_q = {j: v12.get_qpos_joint(model, data, j) for j in v12.FRANKA_JOINTS}
            best_info = {
                "iter": int(it),
                "pos_err": pos_norm,
                "rot_err": rot_norm,
    ...
    best_info["best_err"] = float(best_err)
    return best_q, best_info
...
def resolve_target_frame(target_cfg):
    site_name = target_cfg.get("site", "")
    body_name = target_cfg.get("body", "")
...
def run_one_trial(model, data, candidate, rng, args, trial_idx, viewer=None):
    ...
    target_frame_type, target_frame_name = resolve_target_frame(target_cfg)
    ...
    if abs(pregrasp_z) > 1e-9 or descend_duration > 1e-9:
        q_pre, ik_pre_info = solve_franka_ik_dls_frame(
            model,
            seed_q=seed_q,
            target_frame_type=target_frame_type,
            target_frame_name=target_frame_name,
            target_pos=pregrasp_pos,
            target_quat=pregrasp_quat,
        )

        q_grasp, ik_grasp_info = solve_franka_ik_dls_frame(
            model,
            seed_q=q_pre,
            target_frame_type=target_frame_type,
            target_frame_name=target_frame_name,
    ...
    print("target_frame_type :", target_frame_type)
    print("target_frame_name :", target_frame_name)
...
def main():
    ...
    if args.out:
        ...
        out_path = PROJECT / "diagnostics" / f"fr3_o7_candidate_grasp_site_debug_{candidate['candidate_name']}_{stamp}.json"

    print("\n========== FR3 + O7 CANDIDATE GRASP SITE DEBUG ==========")
    ...
    if args.viewer:
        with mujoco.viewer.launch_passive(model, data) as viewer:
            viewer.cam.lookat[:] = np.array([0.50, 0.0, 0.22])
    ...
    summary = {
        "format": "fr3_o7_candidate_grasp_site_debug_result_v1",
