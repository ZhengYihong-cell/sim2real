...
"""
文件名：
    run_v4_12p4m3_live_contact_gate_debug.py
...
用途：
    ...
    当前问题不是继续评分，而是要验证一个更直接的接触控制策略：
        ...
        先 snapshot 当前所有 7 个手指姿态；
        已冻结手指继续小幅 squeeze；
        未冻结手指保持当前姿态，不允许突然回到 0；
        但进入 squeeze 前必须通过 live-contact gate：当前实时接触必须同时包含 thumb 和至少两根非拇指，并连续稳定若干步。
        可选执行 lift，但只有 live-contact gate 通过后才会进入 squeeze/lift。
...
def all_hand_ctrl_from_qpos(model, data):
    """
    P4M2 阶段切换保护：
    返回当前 7 个 active hand joint 的实际 qpos。
    进入 squeeze 时用它作为所有手指的 base ctrl，
    防止未冻结手指突然被重置回 side_open=0。
    """
    return {j: current_joint(model, data, j) for j in HAND_JOINTS}
...
def run(args):
    ...
    squeeze_base_ctrl_snapshot = None
    final_squeeze_ctrl = None
    live_ready_stable_count = 0

    def sequence(viewer=None):
        nonlocal stop_reason, frozen_ready_step, squeeze_base_ctrl_snapshot, final_squeeze_ctrl, live_ready_stable_count
        ...
        for k in range(probe_steps + 1):
            ...
            # P4M3 live-contact gate:
            # 历史上冻结过某根手指，不等于当前还在接触。
            # 进入 squeeze 前，必须当前实时 contact 同时存在 thumb + 至少 N 根非拇指，
            # 并且连续稳定若干步，物体位移也不能过大。
            live_groups = contact["groups"]
            live_non_thumb = [g for g in NON_THUMB if g in live_groups]
            frozen_history_ready = ("thumb" in frozen_ctrl) and (len(non_thumb_frozen) >= args.min_freeze_non_thumb)
            live_ready_now = (
                frozen_history_ready
                and ("thumb" in live_groups)
                and (len(live_non_thumb) >= args.min_freeze_non_thumb)
                and (disp <= args.max_ready_disp)
            )

            if live_ready_now:
                live_ready_stable_count += 1
            else:
                live_ready_stable_count = 0

            ready = live_ready_stable_count >= args.live_ready_stable_steps

            if k % log_every == 0 or k == probe_steps or ready or live_ready_now:
                row = {
                    ...
                    "live_groups": dict(live_groups),
                    "live_non_thumb": live_non_thumb,
                    "frozen_history_ready": bool(frozen_history_ready),
                    "live_ready_now": bool(live_ready_now),
                    "live_ready_stable_count": int(live_ready_stable_count),
                    "ready": bool(ready),
                    "hand_qpos": hand_qpos(model, data),
                    "contact": contact,
                }
                ...
                print(
                    ...
                    f"frozen={frozen_ctrl} "
                    f"hist_ready={frozen_history_ready} "
                    f"live_ready={live_ready_now} "
                    f"stable={live_ready_stable_count}/{args.live_ready_stable_steps} "
                    f"ready={ready}"
                )
            ...
            if ready:
                ...
                # P4M2 修复：
                # 进入 squeeze 前，记录当前所有 7 个手指姿态。
                # 之后未冻结的 ring/pinky 等手指保持当前姿态，
                # 不再突然回到 side_open_ctrl=0，避免 viewer 中出现“接触后弹飞”。
                squeeze_base_ctrl_snapshot = all_hand_ctrl_from_qpos(model, data)
                for fg, fv in frozen_ctrl.items():
                    fj = GROUP_TO_JOINT[fg]
                    squeeze_base_ctrl_snapshot[fj] = float(fv)

                print(f"[READY] thumb + {len(non_thumb_frozen)} non-thumb groups frozen. Enter squeeze.")
                print(f"[SNAPSHOT] squeeze_base_ctrl_snapshot={squeeze_base_ctrl_snapshot}")
                break
        ...
        if squeeze_base_ctrl_snapshot is None:
            squeeze_base_ctrl_snapshot = all_hand_ctrl_from_qpos(model, data)

        # P4M2 修复：
        # squeeze 的 base 是 ready 瞬间所有手指的实际姿态。
        # 未冻结手指保持当前值；已冻结手指从冻结值继续小幅 squeeze。
        frozen_base_ctrl = dict(squeeze_base_ctrl_snapshot)
        for g, val in frozen_ctrl.items():
            j = GROUP_TO_JOINT[g]
        ...
        for k in range(squeeze_steps + 1):
            ...
            final_squeeze_ctrl = dict(squeeze_ctrl)
        ...
        hold_ctrl = final_squeeze_ctrl if final_squeeze_ctrl is not None else squeeze_ctrl
        ...
        run_phase(
            ...
            lambda a, k: (
                p4f.set_arm_qpos_and_ctrl(model, data, q_grasp),
                p4f.set_hand_ctrl(model, data, hold_ctrl, args.direct_hand_qpos),
            ),
            logs, sets, obj_ref,
        )
        ...
        if args.enable_lift:
            run_phase(
                model, data, viewer, args,
                "lift_after_touch_freeze_squeeze",
    ...
    out = {
        "format": "v4_12p4m3_live_contact_gate_debug",
        ...
        "squeeze_base_ctrl_snapshot": squeeze_base_ctrl_snapshot,
        "final_squeeze_ctrl": final_squeeze_ctrl,
...
def main():
    ...
    # P4M3 live-contact gate:
    # squeeze/lift 之前必须实时保持 thumb + N 根非拇指接触。
    ap.add_argument("--live-ready-stable-steps", type=int, default=30)
    ap.add_argument("--max-ready-disp", type=float, default=0.010)
