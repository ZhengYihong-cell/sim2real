...
"""
文件名：
    run_v4_12p4p_adaptive_wrench_close_debug.py
...
def run(args):
    ...
    print("\n========== V4.12P4P ADAPTIVE WRENCH CLOSE ==========")
    ...
    def step_common(phase, step, total_steps, alpha, arm_cmd, hand_ctrl, adapt=False):
        ...
        if adapt and wrench["hand_object_contacts"]:
            ...
            if not live["contact_ready_now"]:
                dpos = np.asarray(suggestion["suggest_delta_local_xyz"], dtype=float)
                drot = np.asarray(suggestion["suggest_rotvec_local_xyz_rad"], dtype=float)
    ...
    result = {
        "format": "v4_12p4p_adaptive_wrench_close_debug",
