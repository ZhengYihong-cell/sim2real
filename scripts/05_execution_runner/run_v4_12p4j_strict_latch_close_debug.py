...
"""
文件名：
    run_v4_12p4j_strict_latch_close_debug.py

脚本类别：
    debug / runner / strict-stable-force-closure-proxy / support-aware-close / viewer
...
def collect_force_proxy(model, data, sets, object_body, args):
    ...
    non_thumb_group_count = sum(1 for g in NON_THUMB_GROUPS if g in object_groups)

    # Strict latch:
    # 单个 index 被 thumb 推到后接触，不代表已经抓住。
    # 必须至少有 min_latch_non_thumb_groups 个非拇指 group 同时参与。
    force_proxy_ok = bool(
        best_proxy is not None
        and best_proxy["ok"]
        and support_ok
        and non_thumb_group_count >= args.min_latch_non_thumb_groups
    )

    return {
        "ncon": int(data.ncon),
        "object_groups": object_groups,
        "non_thumb_group_count": int(non_thumb_group_count),
...
def stable_force_latch_close(model, data, viewer, args, q_grasp, side_open_ctrl, close_target, sets, obj_ref, logs):
    ...
    for phase_name, steps, interp in [
        ...
        for k in range(steps + 1):
            ...
            # Strict latch:
            # alpha 太小时，四指只闭合了一半，哪怕瞬间 force proxy 达标也不能 latch。
            latch_ready = (
                proxy["force_proxy_ok"]
                and alpha >= args.min_latch_alpha
                and disp <= args.hard_object_push_disp
                and support_safe
            )
            ...
            if stable_count >= args.force_stable_steps:
                ...
                # Strict latch:
                # held_ctrl 用当前命令 ctrl，避免因为 qpos 滞后导致抓握瞬间回弹；
                # 但是否允许 lift，必须由 hold 结束后的 post_hold_force_proxy_ok 决定。
                held_ctrl = dict(ctrl)
                latch_info = {
                    "phase": phase_name,
                    "step": int(k),
                    ...
                    "held_qpos_at_latch": current_hand_qpos_as_ctrl(model, data),
                    "object_disp": disp,
                    "proxy": proxy,
                }
...
def run(args):
    ...
    def sequence(viewer=None):
        ...
        post_hold_proxy = collect_force_proxy(model, data, sets, args.object_body, args)
        post_hold_ok = bool(latch["latched"] and post_hold_proxy["force_proxy_ok"])
        latch["post_hold_proxy"] = post_hold_proxy
        latch["post_hold_force_proxy_ok"] = post_hold_ok

        print("[POST HOLD CHECK] force_proxy_ok:", post_hold_ok, brief_proxy(post_hold_proxy))
...
def main():
    ...
    # Strict latch:
    # 防止四指只闭合一半、或者只有一根非拇指被动接触时就 latch。
    ap.add_argument("--min-latch-alpha", type=float, default=0.78)
    ap.add_argument("--min-latch-non-thumb-groups", type=int, default=2)
