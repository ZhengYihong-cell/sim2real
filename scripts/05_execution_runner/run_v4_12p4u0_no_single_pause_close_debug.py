...
用途：
    V4.12P4U0 no-single-contact-pause close。
    在 P4P/P4P2 的基础上修正一个关键问题：
    单侧接触阶段不允许根据接触力调 hand pose。
    只有 thumb 与至少一个非 thumb 手指同时接触后，才允许使用小幅 hand-local pose adaptation。

输入：
    --model        MuJoCo 场景 XML
    --candidate    候选抓握 JSON
    --p3-json      P3 结果 JSON
    --best-config  P4H/P4E/P4P 选出的 best_config JSON

输出：
    --out          调试结果 JSON，记录 contact group、opposition、cum_dpos_local、cum_drot_local 等

流程位置：
    Top-K candidate / P3 / P4E 后的执行验证阶段。
    用于验证“接触后自适应调姿”是否能形成稳定 thumb-finger 对抗。

不负责：
    不重新做大范围整臂 IK。
    不重新搜索 Top-K 数据集候选。
    不把数据集 qpos 当最终控制命令。
    不在单侧接触阶段强行用接触力方向解释力闭合。
"""
...
def run(args):
    ...
    print("single_contact_adapt_scale:", args.single_contact_adapt_scale)
    print("mixed_contact_adapt_scale :", args.mixed_contact_adapt_scale)
    print("disable_single_contact_pause:", args.disable_single_contact_pause)
    print("====================================================\n")
    ...
    def step_common(phase, step, total_steps, alpha, arm_cmd, hand_ctrl, adapt=False):
        ...
        if adapt and wrench["hand_object_contacts"]:
            ...
            if not args.disable_single_contact_pause:
                ...
                # P4U0 关键实验：
                # 单侧接触不冻结，继续让 thumb/fingers 低速闭合，
                # 用来验证之前的“碰到就冻结”是否破坏了对握形成。
                pause_ctrl.clear()
            ...
            if not live["contact_ready_now"]:
                ...
                groups_now = live.get("groups", {}) if isinstance(live, dict) else {}
                has_thumb = int(groups_now.get("thumb", 0)) > 0
                has_non_thumb = any(
                    int(groups_now.get(k, 0)) > 0
                    for k in ["index", "middle", "ring", "pinky"]
                )
                has_mixed_opposition_contact = bool(has_thumb and has_non_thumb)

                if has_mixed_opposition_contact:
                    adapt_scale = float(args.mixed_contact_adapt_scale)
                    adapt_gate_reason = "mixed_thumb_finger_contact_pose_adapt_enabled"
                else:
                    adapt_scale = float(args.single_contact_adapt_scale)
                    adapt_gate_reason = "single_or_empty_contact_pose_adapt_disabled"

                # P4P3 关键规则：
                # 单侧接触时不根据力方向调 hand pose。
                # 只有形成 thumb + finger 对抗接触后，才允许小幅调姿。
                dpos = float(args.adapt_sign) * adapt_scale * raw_dpos
                drot = float(args.adapt_sign) * adapt_scale * raw_drot

                dpos = limit_vec(dpos, args.max_adapt_delta_per_step)
                drot = limit_vec(drot, math.radians(args.max_adapt_rot_deg_per_step))

                if isinstance(suggestion, dict):
                    suggestion["p4p3_has_thumb"] = has_thumb
                    suggestion["p4p3_has_non_thumb"] = has_non_thumb
                    suggestion["p4p3_has_mixed_opposition_contact"] = has_mixed_opposition_contact
                    suggestion["p4p3_adapt_scale"] = adapt_scale
                    suggestion["p4p3_adapt_gate_reason"] = adapt_gate_reason
    ...
    result = {
        ...
        "disable_single_contact_pause": bool(args.disable_single_contact_pause),
...
def main():
    ...
    # P4P3:
    # 单侧接触阶段不做 hand pose adaptation，避免 thumb 或 finger 单独把物体推飞。
    # 只有 thumb + 非 thumb 同时接触后，才允许小幅位姿修正。
    ap.add_argument("--single-contact-adapt-scale", type=float, default=0.0)
    ap.add_argument("--mixed-contact-adapt-scale", type=float, default=0.20)

    # P4U0:
    # P4P3 会在单侧 thumb / finger 接触时冻结该侧关节。
    # 现在观察到这种冻结会破坏后续对握形成，使物体被单侧推向手心。
    # 本开关用于关闭“单接触冻结”，让手指继续低速闭合。
    ap.add_argument("--disable-single-contact-pause", action="store_true")
