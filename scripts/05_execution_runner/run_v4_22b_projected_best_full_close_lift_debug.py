...
def main():
    ...
    use_projected_grasp = False
    if isinstance(best, dict) and "T_grasp_projected" in best:
        tg = best.get("T_grasp_projected", {})
        if isinstance(tg, dict) and "T" in tg:
            T_grasp = np.asarray(tg["T"], dtype=float)
            use_projected_grasp = True
        else:
    ...
    print("use_projected_grasp:", use_projected_grasp)
    if use_projected_grasp:
        print("projected_dz      :", best.get("dz"))
    ...
    result = {
        ...
        "use_projected_grasp": use_projected_grasp,
        "projected_dz": best.get("dz") if isinstance(best, dict) else None,
