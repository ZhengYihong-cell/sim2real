...
用途：
    V4.12P4U2 slow-arm record demo。
...
def solve_body_position_qpos_dict(model, data, q_seed, target_body_name, target_pos,
                                  ik_iters=180, damping=1e-4, step_scale=0.65):
    """
    P4U2 helper:
        从 q_grasp 附近生成一个录屏用 pregrasp 起点。
        只调整 FR3 7 个关节，使 target_body 到指定 world position。
    """
    q_out = dict(q_seed)
...
def run(args):
    ...
    # P4U2: generate a visible pregrasp for recording.
    q_demo_pre = dict(q_pre)
    pre_ik_info = {"mode": "use_p3_pre", "q_pre_source": "p3_or_fallback"}

    if getattr(args, "slow_arm_mode", "auto_radial") == "auto_radial":
        ...
        hand_pos = data.xpos[target_bid].copy()
        obj_pos = data.xpos[obj_bid].copy()

        radial = hand_pos - obj_pos
        radial[2] = 0.0
        ...
        pre_target_pos = hand_pos + float(args.pre_retreat) * radial + np.array([0.0, 0.0, float(args.pre_lift_z)])

        q_demo_pre, pre_ik_info = solve_body_position_qpos_dict(
            model=model,
            data=data,
            q_seed=q_grasp,
            target_body_name=args.target_body,
            target_pos=pre_target_pos,
            ik_iters=int(args.pre_ik_iters),
            damping=float(args.pre_ik_damping),
        )
        pre_ik_info["mode"] = "auto_radial"
        pre_ik_info["pre_retreat"] = float(args.pre_retreat)
        pre_ik_info["pre_lift_z"] = float(args.pre_lift_z)
        pre_ik_info["radial_dir_world"] = radial

    # Initialize viewer/simulation at pregrasp.
    mujoco.mj_resetData(model, data)
    set_qpos_dict(model, data, q_demo_pre)
    set_qpos_dict(model, data, side_open_ctrl)
    data.qvel[:] = 0.0
    mujoco.mj_forward(model, data)
    ...
    print("slow_arm_mode      :", getattr(args, "slow_arm_mode", None))
    print("pre_ik_info        :", pre_ik_info)
    ...
    def execute(viewer=None):
        ...
        pre_hold_steps = max(0, int(float(args.pre_start_hold_duration) / dt))
        slow_arm_steps = max(1, int(float(args.slow_arm_duration) / dt))
        ...
        runner.run_hold(
            "record_hold_at_pregrasp_side_open",
            pre_hold_steps,
            q_demo_pre,
            side_open_ctrl,
        )
        ...
        runner.run_phase(
            "record_slow_arm_move_pregrasp_to_grasp",
            slow_arm_steps,
            q_demo_pre,
            q_grasp,
            side_open_ctrl,
...
def main():
    ...
    # P4U2 record-demo arm motion.
    ap.add_argument("--slow-arm-mode", choices=["auto_radial", "use_p3_pre"], default="auto_radial")
    ap.add_argument("--slow-arm-duration", type=float, default=5.0)
    ap.add_argument("--pre-start-hold-duration", type=float, default=1.0)
    ap.add_argument("--pre-retreat", type=float, default=0.12)
    ap.add_argument("--pre-lift-z", type=float, default=0.03)
    ap.add_argument("--pre-ik-iters", type=int, default=180)
    ap.add_argument("--pre-ik-damping", type=float, default=1e-4)
