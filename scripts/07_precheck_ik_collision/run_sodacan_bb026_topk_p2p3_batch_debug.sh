...
#
# 用途：
#   对第二个 SodaCan 泛化目标 sem-SodaCan-bb0262b63f857b24dc4868f575aa7e3c
#   批量执行 candidate/scene 生成、P2 Pinocchio 多 seed IK、P3 MuJoCo 碰撞预检。
#
# 输入：
#   dataset/O7_Full_V8BestBaseline_165objs_20260422_084834/validate_results/seed1/
#     sem-SodaCan-bb0262b63f857b24dc4868f575aa7e3c.npy
#
# 输出：
#   diagnostics/current_v412/sodacan_bb026_topk_p2p3_batch_debug/
#     sampleXXX/
#       initial_debug/
#       sampleXXX_p2.json
#       sampleXXX_p3.json
...
#   1. 不运行 viewer；
#   2. 不执行动态抓取；
#   3. 不修改已固化的 can52 / sodacan165 demo；
#   4. 不重新搜索数据集；
#   5. 不手工调单个 sample。
...
TARGET="sem-SodaCan-bb0262b63f857b24dc4868f575aa7e3c"
NPY="dataset/O7_Full_V8BestBaseline_165objs_20260422_084834/validate_results/seed1/${TARGET}.npy"
OBJECT_MESH="dataset/meshdata/${TARGET}/coacd/decomposed.obj"

OUTROOT="diagnostics/current_v412/sodacan_bb026_topk_p2p3_batch_debug"
mkdir -p "$OUTROOT"

LOG_ALL="$OUTROOT/terminal_all.txt"
exec > >(tee "$LOG_ALL") 2>&1

# 来自 next_can_target_report 的推荐 Top sample。
SAMPLES="${SAMPLES:-8 36 10 6 14 9 11 26}"
...
for f in "$BUILDER" "$P2_SCRIPT" "$P3_SCRIPT" "$URDF" "$NPY" "$OBJECT_MESH"; do
  if [[ ! -f "$f" ]]; then
...
echo "========== SODACAN BB026 TOP-K P2/P3 BATCH =========="
echo "target      : $TARGET"
echo "npy         : $NPY"
echo "object_mesh : $OBJECT_MESH"
...
for IDX in $SAMPLES; do
...

  {
    ...
    MODEL="$(find "$SDIR/initial_debug/scene" -type f -name "*.xml" | head -1)"
    ...
    if [[ ! -f "$MODEL" ]]; then
      echo "[ERROR] model xml not generated for sample${SID}"
      exit 3
    fi
    if [[ ! -f "$CANDIDATE" ]]; then
      echo "[ERROR] candidate json not generated for sample${SID}"
      exit 3
    fi
...
outroot = Path("diagnostics/current_v412/sodacan_bb026_topk_p2p3_batch_debug")
rows = []
...
def sort_key(r):
    ...
    if fo is None:
        fo = -999
    hsc = r.get("static_grasp_closed_hand_support_clearance")
    if hsc is None:
        hsc = -999
    score = r.get("best_available_score")
    if score is None:
        score = 1e99
    return (-num_pass, -hs, -fo, -hsc, score)

rows_sorted = sorted(rows, key=sort_key)

summary = {
    "format": "sodacan_bb026_topk_p2p3_batch_summary_debug_v1",
    "outroot": str(outroot),
...
lines = []
lines.append("========== SODACAN BB026 TOP-K P2/P3 BATCH SUMMARY ==========")
