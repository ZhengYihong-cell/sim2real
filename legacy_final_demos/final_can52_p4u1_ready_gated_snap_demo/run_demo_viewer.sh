...
./run_mujoco_clean.sh scripts/05_execution_runner/run_v4_12p4u1_precontact_snap_close_debug.py \
  ...
  --out outputs/can52_p4u1_ready_gated_snap_micro_squeeze_lift_viewer.json \
  --viewer \
  --keep-viewer-open \
  --move-steps 80 \
  ...
  --log-every-steps 30 \
  --frame-sleep 0.001 \
  2>&1 | tee outputs/can52_p4u1_ready_gated_snap_micro_squeeze_lift_viewer_terminal.txt
